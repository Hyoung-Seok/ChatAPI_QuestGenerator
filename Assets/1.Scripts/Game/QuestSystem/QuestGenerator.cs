...
public class QuestGenerator
{
    ...
    public QuestGenerator(EChatModel model, float temperature)
    {
        ...
        _systemMessage = "너는 게임의 퀘스트를 JSON형식으로 생성하는 어시스던트야. 아래와 같은 형태로 JSON을 생성해줘." +
                         ...
                         "Title : 퀘스트의 제목." +
                         "Type : 퀘스트 타입. 전투(Fight), 얻어오기(Get), 전달하기(Deliver)가 있어. 퀘스트 타입에 맞춰서 스크립트를 생성해줘." +
                         "만약 퀘스트의 타입이 Deliver(전달하기)라면, 전해주는 대상한테 퀘스트를 완료할 수 있도록 Deliver타입의 퀘스트를 하나 더 만들어줘." +
                         "하나 더 만드는 퀘스트는 단순 완료용 퀘스트이므로, 스크립트에 다른 목표를 만들지 마." +
